...
public class RenderingHelper {
  ...
  **/
  public static com.jme3.math.Vector2f getScreenSpacePos(
    ...
    diffMC.transform(Vector3f.YP.rotationDegrees(camera.getYRot() + 180.0F));
    diffMC.transform(Vector3f.XP.rotationDegrees(camera.getXRot()));
    ...
    Matrix4f projM = Minecraft.getInstance().gameRenderer.getProjectionMatrix(
      Minecraft.getInstance().gameRenderer.getFov(camera, partialTicks, true)
    );
...
//    Vector4f d4 = new Vector4f(worldPos.x(), worldPos.y(), worldPos.z(), 1.0f);
//    d4.transform(matrix);
//    // Now diff is in clip space.
//    d4.perspectiveDivide();
//    // now diff is NDC.
//    d4.mul(new Vector3f(scaledWidth / 2, scaledHeight / 2, 1));
//    com.jme3.math.Vector2f ssPos = new com.jme3.math.Vector2f(d4.x(), d4.y());
//    if (backwards) {
//      // multLocal(1000) ensures the gatePos appear off screen;
//      ssPos.multLocal(-1).multLocal(1000);
//    }
